...
public class Panel {
	...
	public int updateRainbow(int IN) {
		...
		hue2 += NewGui.rainbowspeed.getValue() / 1000f;
		if (hue2 > 1) hue2 -= 1;
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		//if (NewGui.outline.getValue()) XuluTessellator.drawRectOutline(x - 2, y - 2, x + width + 2, y + width, x, y, x + width, y + width, ColorUtils.changeAlpha(outlineColor, 100));
		...
		*/
		...
		if (this.extended && !Elements.isEmpty()) {
			...
			int epanelcolor = ColorUtils.changeAlpha(ColorUtils.Colors.BLACK, NewGui.bgAlpha.getValue()); // 0xbb151515
			...
			for (ModuleButton et : Elements) {
				...
				*/
				...
				if (NewGui.moduleSetting.getValue().equalsIgnoreCase("MiniButton")) Gui.drawRect((int)x + 2, 	(int)startY + 1, (int)(x + width) - 2, (int)(startY + et.height), mcolor);
				et.x = x + 2;
				et.y = startY;
				et.width = width - 4;
