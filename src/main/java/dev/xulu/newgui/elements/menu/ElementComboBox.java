...
public class ElementComboBox extends Element {
	...
	if (comboextended) {
		...
		for (String sld : set.getOptions()) {
			...
			if (NewGui.customfont.getValBoolean()) {
			...
			if (sld.equalsIgnoreCase(set.getValString())) {
				Gui.drawRect((int)x, (int)ay, (int)(x + 1.5), (int)(ay + (NewGui.customfont.getValBoolean() ? Eclient.cFontRenderer.getHeight() : FontUtil.getFontHeight()) + 2), clr1);
			}
			*/ /*
			if (mouseX >= x && mouseX <= x + width && mouseY >= ay && mouseY < ay + (NewGui.customfont.getValBoolean() ? Eclient.cFontRenderer.getHeight() : FontUtil.getFontHeight()) + 2) {
				Gui.drawRect((int)(x + width - 1.2), (int)ay, (int)(x + width), (int)(ay + (NewGui.customfont.getValBoolean() ? Eclient.cFontRenderer.getHeight() : FontUtil.getFontHeight()) + 2), clr2);
			}
			ay += (NewGui.customfont.getValBoolean() ? Eclient.cFontRenderer.getHeight() : FontUtil.getFontHeight()) + 2;
		}
	}
	*/
	...
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		if (mouseButton == 0) {
			if (isButtonHovered(mouseX, mouseY)) {
				try {
					if (!set.getCorrectString(s).equalsIgnoreCase(set.getOptions().get(set.getOptions().size() - 1).toString())) {
						set.setValue(set.getOptions().get(set.getOptions().indexOf(set.getCorrectString(s)) + 1));
					} else {
		...
		if (mouseButton == 1) {
			if (isButtonHovered(mouseX, mouseY)) {
				try {
					if (set.getOptions().listIterator(set.getOptions().indexOf(set.getCorrectString(s))).hasPrevious())
						set.setValue(set.getOptions().listIterator(set.getOptions().indexOf(set.getCorrectString(s))).previous());
					else
						set.setValue(set.getOptions().get(set.getOptions().size() - 1));
				} catch (Exception e) {
					System.err.println("Error with invalid combo");
					e.printStackTrace();
					set.setValue(set.getOptions().get(0));
